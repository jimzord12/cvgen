...
// document-shell
#show: body => if legacy {L.document-shell(d, theme, artwork, layout, body)} else {C.document-shell(ctx, d, body)}
...
#grid(columns: (1fr, 1fr, 1fr), ..("8", "7", "1").zip(("Contracts", "Vessels", "Company")).map(((v, c)) =>
  if legacy {L.metric(v, c, theme)} else {C.metric(ctx, v, caption: c)}))
#v(4mm)
#if legacy {L.skills-section((("Diesel overhaul", "Purifiers"), ("Planned maintenance",)), theme, geometry: skills-geo)} else {
  F.skills-section(skills-ctx, (("Diesel overhaul", "Purifiers"), ("Planned maintenance",)))}
...
#if legacy {L.decoration(artwork.profile-illustration, theme, width: 30mm)} else {C.decoration(ctx, artwork.profile-illustration, width: 30mm)}
...
#block(width: layout.width, height: 30mm, clip: true, if legacy {L.page-background(theme, artwork, layout)} else {C.page-background(ctx)})
