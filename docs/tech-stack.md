...
| Data | JSON read with `json()` | | Editable by anyone, validated by `packages/cv-engine/domains/marine/schema/candidate.schema.json` and by assertions in `packages/cv-engine/domains/marine/data.typ` (over the common checks in `packages/cv-engine/core/data.typ`) |
