...
| Fonts | Source Sans 3, Barlow Condensed (family string `Barlow`), Cormorant Garamond, GFS Didot (Greek Font Society; the `Text Draft`'s display face, sets Greek and Latin) | bundled, OFL | Reproducible renders on any machine. Passed with `--font-path packages/cv-framework/fonts` |
